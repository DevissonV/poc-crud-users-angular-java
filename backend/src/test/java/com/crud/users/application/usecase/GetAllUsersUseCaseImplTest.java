...
@DisplayName("GetAllUsersUseCase - Unit Tests")
class GetAllUsersUseCaseImplTest {
    ...
    void shouldGetAllUsers() {
        given(userRepositoryPort.findAll()).willReturn(Arrays.asList(mockUser));
        ...
        verify(userDtoMapper).toResponseDto(mockUser);
    }

    @Test
    @DisplayName("Should return empty list when no users exist")
    void shouldReturnEmptyListWhenNoUsers() {
        given(userRepositoryPort.findAll()).willReturn(Arrays.asList());
